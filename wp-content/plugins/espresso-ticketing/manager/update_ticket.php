...
function update_event_ticket(){
	...
	$template_file= isset($_REQUEST['template_file']) && !empty($_REQUEST['template_file']) ? $_REQUEST['template_file'] : '';
	...
	if ($wpdb->update( EVENTS_TICKET_TEMPLATES, $sql, $update_id, $sql_data, array( '%d' ) )){
		$notices['updates'][] = __('The ticket has been updated', 'event_espresso');
	}else { 
